...
export default function WorkshopCheckin(): React.ReactNode {
  ...
  return <main className={styles.page}><WorkshopTabs />
    ...
    {state === 'anonymous' && <section className={styles.state}><LogIn /><strong>登录后查看任务</strong><span>登录并开通 HackStart 永久会员后，可以提交 Workshop 打卡。</span><a href={loginHref}>登录 / 注册</a></section>}
    {state === 'forbidden' && <section className={styles.state}><LockKeyhole /><strong>需要 HackStart 永久会员</strong><span>会员可以阅读专属课程、完成任务并把成果公开到社区。</span><Link to="/membership/">了解永久会员</Link></section>}

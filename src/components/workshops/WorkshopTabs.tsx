...
export default function WorkshopTabs(): React.ReactNode {
  ...
  return <>
    <nav className={styles.tabs} aria-label="Workshop 功能">{tabs.map(({href, label, icon: Icon, active}) => <Link key={label} to={href} className={active ? styles.tabActive : undefined} aria-current={active ? 'page' : undefined}><Icon aria-hidden="true" />{label}</Link>)}</nav>
